...
beforeAll(() => {
  ...
  writeFileSync(
    ...
    JSON.stringify({
      ...
      rules: {
        "f0-stories/no-nondeterministic-story-accessible-names": "error",
      },
    })
  )
...
const lintAs = (name: string, source: string): string[] => {
  ...
  return [
    ...output.matchAll(
      /f0-stories\(no-nondeterministic-story-accessible-names\): ([^\n]+)/g
    ),
  ].map((match) => match[1])
}
...
describe("no-nondeterministic-story-accessible-names", () => {
