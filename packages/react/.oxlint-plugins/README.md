...
- **`no-nondeterministic-story-accessible-names`** — a story's _accessible names_ must be
  ...
  mock widgets a story imports render into the same tree. An `"error"`: the
  tree was clean when it shipped, so there is no debt to ratchet down.
