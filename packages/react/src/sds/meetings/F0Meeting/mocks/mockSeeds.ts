...
export type MockPerson = {
  ...
  camera?: boolean
  muted?: boolean
  /** Letterboxed instead of cropped — see `F0MeetingParticipant.preventCrop`. */
  preventCrop?: boolean
  /**
...
export type MockMeetingSeed = {
  ...
  transcript?: boolean
  /**
   * Really open the microphone when the call connects unmuted, instead of
   * only on the first unmute. What makes the tab a call to the browser:
   * Chromium's automatic picture-in-picture fires only for a page capturing
   * camera or microphone. Off by default — a docs page of twenty rooms must
   * not ask for the mic twenty times.
   */
...
const ROSTER: MockPerson[] = [
  ...
  person("p30", "Elena", "Vázquez"),
  person("p31", "Jonas", "Weber", { camera: false }),
  person("p32", "Aisha", "Rahman"),
  person("p33", "Mateo", "Serrano", { muted: true }),
  person("p34", "Chloé", "Dubois"),
  person("p35", "Ravi", "Menon", { camera: false }),
  person("p36", "Freya", "Olsen"),
  person("p37", "Daniel", "Kaplan"),
  person("p38", "Mei", "Chen", { muted: true }),
  person("p39", "Oskar", "Nowak"),
  person("p40", "Leila", "Haddad", { camera: false }),
  person("p41", "Gabriel", "Costa"),
  person("p42", "Hanna", "Virtanen"),
  person("p43", "Idris", "Bello", { camera: false }),
  person("p44", "Nora", "Fischer"),
  person("p45", "Sebastián", "Ortega", { muted: true }),
  person("p46", "Yara", "Saleh"),
  person("p47", "Felix", "Brandt", { camera: false }),
  person("p48", "Ingrid", "Solberg"),
  person("p49", "Tariq", "Hussain"),
]
...
export const fiftyPeopleSeed = seed("Company all hands", 49)

/**
 * A room of `count` people, you included. Names follow the roster, so the same
 * count is the same room every time — which is what a count control needs.
 */
export const roomOf = (
  count: number,
  overrides: Partial<MockMeetingSeed> = {}
  ...
  seed(
    `Room of ${count}`,
    Math.max(0, Math.min(ROSTER.length, count - 1)),
    overrides
  )

/** Names that do not fit: the label has to truncate, never wrap or overflow. */
export const longNamesSeed = seed("Quarterly planning", 0, {
  others: [
    person("l1", "María de los Ángeles", "Fernández-Castellanos"),
    person("l2", "Bartholomew", "Montgomery-Featherstonehaugh", {
      camera: false,
    }),
    person("l3", "Nomvula Thandeka", "Mokoena-Van der Westhuizen"),
    person("l4", "Konstantinos", "Papadimitrakopoulos", { muted: true }),
    person("l5", "Anna-Katharina", "Schwarzenberger-Müller", {
      camera: false,
    }),
  ],
})
