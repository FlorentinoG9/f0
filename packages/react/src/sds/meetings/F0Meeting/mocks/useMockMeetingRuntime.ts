...
export const useMockMeetingRuntime = (
  ...
  // A real call captures the microphone from the moment it connects, muted or
  // not — and that capture is what tells Chromium the tab is a call worth
  // following into picture-in-picture. `leave` releases the stream, so the
  // next room in the same hook asks again.
  const captureMicrophone = seed.captureMicrophone ?? false
  useEffect(() => {
    if (
      !captureMicrophone ||
      status !== "connected" ||
  ...
  const participants = useMemo<F0MeetingParticipant[]>(() => {
    ...
    const remotes = members.map<F0MeetingParticipant>((member, index) => {
      ...
      return {
        ...
        preventCrop: member.preventCrop,
