...
export type SyntheticVideoOptions = {
  /** `false` paints one frame and never animates — required for snapshots. */
  animated?: boolean
}

/**
 * `captureStream(0)` never pushes a frame on its own: one has to be asked for,
 * and a frame captured before the element has become a sink is lost. So ask
 * now and again shortly after, which covers the element's asynchronous load.
 */
const deliverStaticFrame = (stream: MediaStream): (() => void) => {
  const track = stream.getVideoTracks()[0] as
    | (MediaStreamTrack & { requestFrame?: () => void })
    | undefined
  const request = (): void => track?.requestFrame?.()
  request()
  const timers = [50, 300].map((delay) => setTimeout(request, delay))
 ...
 *
 * `animated: false` freezes it on its first frame, for snapshots.
 */
export const createScreenShareBinding = ({
  animated = true,
}: SyntheticVideoOptions = {}):
  ...
  // One frame up front, so a frozen share is a page of code and not black.
  paint(0)
