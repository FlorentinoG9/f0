...
const useStableCapabilities = (
  ...
  const resolved = same ? previous : capabilities
  // Committed, not rendered: a render React throws away must not move the
...
export const F0MeetingProvider = ({
  ...
  // Synced on commit. Children's layout effects run before this one, so
  // `useTrackBinding` reads the track it was handed first and this map second.
  useLayoutEffect(() => {
...
/**
 * The roster, or `null` when there is no call — for the surface provider,
 * which renders with or without one (see {@link F0Meeting}).
 */
export const useF0MeetingRosterOptional = (): F0MeetingRoster | null =>
  useContext(F0MeetingRosterContext)
