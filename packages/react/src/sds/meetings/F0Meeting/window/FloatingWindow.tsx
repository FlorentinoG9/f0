...
import { modeTransitionCss } from "../utils/meeting-motion"
...
export const FloatingWindow = ({ header, children }: FloatingWindowProps) => {
  ...
  const handleKeyDown = useCallback(
    (event: React.KeyboardEvent) => {
      // Only the title bar's own focus moves the window. A host control in
      // `extra` (a select, a slider) keeps its arrow keys.
      if (!canManipulate || event.target !== event.currentTarget) {
        return
      }
  ...
  // mode — including full screen, where it is the viewport. Full screen used
  // to be pinned to `regular`, which on a phone drew desktop chrome into a
  // 360px-wide room.
  const density = densityFor(rect)
  ...
  return (
    <MeetingDensityProvider density={density}>
      <div
        ...
        style={{
          transition:
            isDragging || shouldReduceMotion ? "none" : modeTransitionCss,
      ...
      >
        <div
          ...
          style={{
            ...
            // A pill is all title bar. Everything else, full screen included,
            // takes the height its density asks for — one table, no `h-14`
            // special case to keep in step with it.
            ...(isMinimized ? {} : { height: HEADER_HEIGHT[density] }),
          }}
          className={cn(
            "relative flex shrink-0 items-center gap-2",
            density === "tight" ? "px-2" : isFullscreen ? "px-4" : "px-3",
            isMinimized && "h-full",
