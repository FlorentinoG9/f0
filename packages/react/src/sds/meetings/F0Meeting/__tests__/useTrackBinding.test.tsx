...
describe("useTrackBinding", () => {
  ...
  it("attaches a binding that arrives after its key", () => {
    // A stream that opens late: the track is announced first, its binding a
    // transport event later, under the SAME key. Keying the effect on the key
    // alone left that element blank for the rest of the call.
    const track: F0MeetingTrack = {
      id: "t",
      kind: "camera",
      bindingKey: "t:0",
    ...
    const binding = vi.fn(() => vi.fn())
    const bound: F0MeetingTrack = { ...track, binding }
    rerender(
      <F0MeetingProvider runtime={buildRuntime(bound)}>
        <Video track={bound} />
      </F0MeetingProvider>
    )
    ...
    // And not again on the next ordinary rebuild.
    const churned: F0MeetingTrack = { ...bound, binding: vi.fn(() => vi.fn()) }
    rerender(
      <F0MeetingProvider runtime={buildRuntime(churned)}>
        <Video track={churned} />
      </F0MeetingProvider>
  ...
  it("detaches when the binding is withdrawn under the same key", () => {
    const detach = vi.fn()
    const binding = vi.fn(() => detach)
    const track: F0MeetingTrack = {
      id: "t",
    ...
    const withdrawn: F0MeetingTrack = { ...track, binding: undefined }
    rerender(
      <F0MeetingProvider runtime={buildRuntime(withdrawn)}>
        <Video track={withdrawn} />
      </F0MeetingProvider>
    )
