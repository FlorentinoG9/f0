...
import { planGrid } from "../layout/plan-grid"
import { solveSpotlight } from "../layout/spotlight-solver"

/** The room's own pipeline: `MeetingGrid` calls exactly this. */
const plan = (count: number, width: number, height: number) => {
  const result = planGrid({ box: { width, height }, tileCount: count })
  return {
    capacity: result.capacity,
    hasOverflow: result.capacity < count,
    visible: result.visibleCount,
    cells: result.rects.length + (result.chipRect ? 1 : 0),
 ...
 *
 * Sixteen is `GRID_MAX_TILES`: the desktop rooms would seat more and the cap
 * is what stops them. The floating window is the floors at work.
 */
const CONTAINERS = [
  ...
  { name: "floating 360", width: 360, height: 224, expected: 12 },
]
...
describe("grid capacity", () => {
  ...
  it("never seats more than the cap, and the chip comes out of it", () => {
    // A 4K display could seat forty at the floors. It shows fifteen and a
    // "+25": the cap is a cell count, and the chip is one of the cells.
    const { visible, cells, capacity } = plan(40, 3840, 2160)
    expect(capacity).toBe(GRID_MAX_TILES)
    expect(visible).toBe(GRID_MAX_TILES - 1)
    expect(cells).toBe(GRID_MAX_TILES)
  })
