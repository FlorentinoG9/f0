...
// A room that measures 0x0 lays nothing out. Give it a real box so the tests
// below run against a grid with tiles in it, not an empty stage.
beforeEach(() => {
  measure(1280, 720)
})
...
describe("mode switch", () => {
  ...
  it("never remounts the video moving between window modes", async () => {
    ...
    // The probe and the grid's own tile each attach once at mount.
    const attachedAtMount = binding.mock.calls.length
    expect(attachedAtMount).toBeGreaterThan(0)
    ...
    // its portal, so the elements the track is attached to survive all of
    ...
    expect(binding).toHaveBeenCalledTimes(attachedAtMount)
  })
